...
500 ' ***** PLAYER CONTROL *****
510 IF JF THEN Y(0)=Y(0)+JY(JF):JF=JF-1:S=JS:IF JF GOTO 540 ELSE IF TL=0 THEN GS=4:GOTO 540 ELSE GOTO 540
...
820 Y(N)=3088:GOSUB 250:SF=0
830 FOR I=3 TO 5:IF ABS(X(N)-X(I))<192 AND ABS(Y(N)-Y(I))<192 THEN GOSUB 860
840 NEXT I:IF SC=0 THEN GS=6 ELSE SC=SC-10:GOSUB 2400
850 RETURN
860 SC=SC+(MC*10):TL=TL-1:GOSUB 2400:PLAY "T255L32O5V12CGEB":F(I)=0:Y(I)=3088:SWAP N,I:GOSUB 250:SWAP N,I
870 RETURN
...
1170 IF F(N) AND INT(RND(1)*5)=0 THEN LOCATE X(N)\128+1,Y(N)\128+1:PRINT "n";
1180 RETURN
...
1700 ' ***** MISS *****
1710 IF TL=0 THEN RETURN
1720 GOSUB 8600:JF=1:P(0)=8:R(0)=0:GOSUB 150
1730 Y(0)=Y(0)-JY(JF):GOSUB 200:JF=JF-(JF<7):IF Y(0)<3072 THEN 1730
...
3080 GOSUB 8750:Y(N)=3088:GOSUB 270:GOSUB 2300
3090 GS=3:RETURN

3500 ' ***** ROUND CLEAR *****
3510 GOSUB 6030:X1=X(0)/128:X2=30:P(0)=0:GOSUB 5400
3520 LOCATE 9,8:PRINT USING "ROUND # CLEAR";RD
3530 LOCATE 10,10:PRINT USING "BONUS #####";RD*100
3540 GOSUB 8800:SC=SC+RD*100:IF RD=5 THEN GS=5 ELSE RD=RD+1:GS=2
3550 RETURN

4000 ' ***** ALL ROUND CLEAR *****
4010 CLS:GOSUB 8000:LOCATE 8,5:PRINT "CONGRATULATIONS!!";
4020 GOSUB 8900:SC=SC+1000
4030 GS=1:RETURN
...
5170 GOSUB 2200:RD=1:LF=2:SC=100:CF=1:GS=2:RETURN

5300 ' ----- GAME INIT -----
5310 C=1:SF=0:BF=0:JF=0:JS=0:IF CF THEN TL=((RD*2)+3):CF=0
...
8200 LOCATE 0,22:PRINT " SCORE        ROUND      LEFT";:GOSUB 2400
