...
430 X=X(0)\16:Y=Y(0)\16:FOR I=4 TO 7:IF F(I) AND ABS((X(I)\16)-X)<10 AND ABS((Y(I)\16)-Y)<10 THEN GS=6
440 NEXT I
...
820 SF=0:X=X(1)\16+4:Y=Y(1)\16+4:FOR I=3 TO 5:IF ABS(X(I)\16-X)<24 AND ABS(Y(I)\16-Y)<20 THEN GOSUB 850
...
2120 GOSUB 270:TF=0:IF ABS(X(N)-TX(N))<256 AND ABS(Y(N)-TY(N))<256 THEN X(N)=TX(N):Y(N)=TY(N):TF=1
2130 RETURN
