...
420 A=&H1800+(X(0)\128)+(Y(0)\128)*32:IF VPEEK(A)<>32 OR VPEEK(A-32)<>32 THEN GS=6:RETURN
430 X=X(0):Y=Y(0):FOR I=4 TO 7:IF F(I) AND ABS(X(I)-X)<176 AND ABS(Y(I)-Y)<176 THEN GS=6:NEXT I ELSE NEXT I
...
820 SF=0:X=X(1):Y=Y(1):FOR I=3 TO 5:IF ABS(X-X(I))<192 AND ABS(Y-Y(I))<192 THEN GOSUB 850
830 NEXT I:IF SC=0 THEN GS=6 ELSE SC=SC-10:GOSUB 2400
840 Y(N)=3088:GOSUB 270:RETURN
850 SC=SC+(MC*10):GOSUB 2400:TL=TL+(TL>0):PLAY "T255L32O5V12CGEB":F(I)=0:Y(I)=3088:SWAP N,I:GOSUB 270:SWAP N,I
860 RETURN
...
1170 IF RD>1 AND INT(RND(1)*50)<RD*4 THEN LOCATE X(N)\128,Y(N)\128+1:PRINT "n";
1180 RETURN

1200 X=X(N):Y=Y(N):IF INT(RND(1)*30)<RD*2 THEN 1220
...
3500 ' ***** ROUND CLEAR *****
3510 FOR N=3 TO 7:Y(N)=3088:X(N)=128:GOSUB 270:NEXT N:GOSUB 2300
