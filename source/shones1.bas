...
430 X=X(0)\16:Y=Y(0)\16:FOR I=4 TO 7:IF F(I) AND ABS(X(I)\16-X)<16 AND ABS(Y(I)\16-Y)<12 THEN LOCATE 0,21:PRINT I;INT(X(I)\16);INT(Y(I)\16);X;Y;:GS=6
440 NEXT I
...
820 SF=0:X=X(1)\16:Y=Y(1)\16:FOR I=3 TO 5:IF F(I) AND ABS(X(I)\16-X)<16 AND ABS(Y(I)\16-Y)<16 THEN GOSUB 850
...
1160 IF RD>1 AND INT(RND(1)*100)<(RD-1)*5 THEN LOCATE X(N)\128,Y(N)\128+1:PRINT "n";
1170 RETURN
...
1555 LOCATE 0,21:PRINT N;INT(X(N)\16);INT(Y(N)\16);
1560 RETURN
...
2030 WX(N)=INT((DX/A)*(8*SP)*16):WY(N)=INT((DY/A)*(8*SP)*16)
2040 RETURN
...
5170 GOSUB 2200:RD=3:LF=2:SC=100:TL=0:GS=2:RETURN
